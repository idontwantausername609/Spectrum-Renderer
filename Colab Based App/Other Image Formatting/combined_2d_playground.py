...
def bar_iteration():
    ...
    for index, row in df_filtered.iterrows():
        ...
        final_intensity_scale = new_utils.final_scale(new_utils.min_bright, normalized_intensity)
        color_rgb = new_utils.colored_rgb(base_rgb, final_intensity_scale)
    ...
    if GRAPH_TYPE == 'bar' or GRAPH_TYPE == 'b':
        ...
        if FILL_TYPE == 'yes' or FILL_TYPE == 'y':
            ...
            if PLOT_TYPE == 'yes' or PLOT_TYPE == 'y':
                ...
                filled_plot()
                plt.title("Smoothed Plot", color = "white")
            else:
...
new_utils.axis_labels()       # reverse_x input not working properly - fix: put plt.xlim inside "if reverse_x == 'yes' " loop
plot_graph()
plt.show() 